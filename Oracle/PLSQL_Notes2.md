...
- The Oracle server allocates a private memory area calledacontext area to store the data 
...
1. Declare a cursor.
    - The cursor in the declaraative section is defined by the SQL `select` statement.
    - Do not include the `into` clause in the cursor declaration because it appears
    later in the `fetch` statement. 
    - If processing rows in a specific sequence is required, then  usethe `orderby` 
    clause in the query.
...
4. Define `exit` condition to terminate the loop.
    ...
    ```
5. `close` the staements to control a cursor. Release the active set.
...
#### Gudelines for Fetching data from the Cursor
- Include the same number of variables in the `into` clause  of the `fetch` statement as 
columns in the `select`  statement, and be sure that the data types arecompatible.
...
- You can use the `%notfound` cursor attribute to test for  the exitcondition.
